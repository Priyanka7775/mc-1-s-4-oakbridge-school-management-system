...
public class OakBridgeSchool {
        public int[] calculateTotalMarks(int [] math,int science[],int[] english) {
            ...
            public double calculateAverageScienceMarks ( int[] science, int len){
                ...
                double avgTillLast = calculateAverageScienceMarks(science, len - 1);
                double total = (avgTillLast * (len - 1)) + science[len - 1];
                return total / len;
            }

            //Write the logic to find the top score in the class using recursion
            public int findTopScore ( int[] totalMark, int len){
                if (len == 1)
                    return totalMark[len - 1];
                if (findTopScore(totalMark, len - 1) > totalMark[len - 1])
                    return findTopScore(totalMark, len - 1);
                else {
                    return totalMark[len - 1];
                }
